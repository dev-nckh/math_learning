...
export default function LessonScreen() {
  ...
  const lessonData = lessonsData[chapterNum as keyof typeof lessonsData]?.[lessonNum as keyof typeof lessonsData[keyof typeof lessonsData]];

  const navigateToGame = (gameId: number) => {
    console.log(`🎮 Navigating to Chapter ${chapterNum}, Lesson ${lessonNum}, Game ${gameId}`);
    router.push(`/(menu)/chapters/${chapterNum}/lessons/${lessonNum}/games/${gameId}` as any);
  };

  const navigateToGamesOverview = () => {
    router.push(`/(menu)/chapters/${chapterNum}/lessons/${lessonNum}/games` as any);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Header */}
        <TouchableOpacity style={styles.backButton} onPress={goBack}>
          <Text style={styles.backButtonText}>← Quay lại Chapter {chapterNum}</Text>
        </TouchableOpacity>
        ...
        <View style={styles.gamesSection}>
          ...
          {lessonData.games.map((game) => (
            <TouchableOpacity 
              key={game.id}
              style={[styles.gameCard, { borderLeftColor: getGameTypeColor(game.type) }]}
            ...
            >
              ...
              <View style={styles.gameFooter}>
                <View style={[styles.typeBadge, { backgroundColor: getGameTypeColor(game.type) }]}>
