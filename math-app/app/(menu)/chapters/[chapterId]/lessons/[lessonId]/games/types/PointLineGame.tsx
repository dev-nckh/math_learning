...
export default function PhysicsBallGame({
  ...
  useEffect(() => {
    if (gamePhase === "game") {
      ...
      const basketPositions = [
        ...
        {
          x: drawAreaWidth * 0.8 - BASKET_WIDTH,
          y: drawAreaHeight - BASKET_HEIGHT,
        }, // Level 2: Right side
        {
          x: drawAreaWidth * 0.5 - BASKET_WIDTH / 2,
          y: drawAreaHeight - BASKET_HEIGHT,
        }, // Level 3: Center
        { x: drawAreaWidth * 0.1, y: drawAreaHeight - BASKET_HEIGHT }, // Level 4: Far left
        {
          x: drawAreaWidth * 0.9 - BASKET_WIDTH,
          y: drawAreaHeight - BASKET_HEIGHT,
        }, // Level 5: Far right
      ];
  ...
  const updateBallPhysics = (ball: Ball, deltaTime: number): Ball => {
    ...
    if (
      Math.abs(newBall.velocityY) < MIN_VELOCITY &&
      newBall.y + newBall.radius >= drawAreaHeight - 1
    ) {
      newBall.velocityY = 0;
    }
  ...
  const checkBallLineCollision = (
    ...
    const projection =
      (ballToStart.x * dx + ballToStart.y * dy) / (length * length);
    ...
    if (distance <= ball.radius && distance > 0) {
      ...
      const velocityDotNormal =
        newBall.velocityX * normalX + newBall.velocityY * normalY;
      ...
      if (velocityDotNormal < 0) {
        ...
        const velocityDotTangent =
          newBall.velocityX * tangentX + newBall.velocityY * tangentY;
  ...
  const startGameLoop = () => {
    ...
    const gameLoop = () => {
      ...
      setGameState((prev) => {
        ...
        if (
          Math.abs(updatedBall.velocityX) > MIN_VELOCITY ||
          Math.abs(updatedBall.velocityY) > MIN_VELOCITY
        ) {
          ...
          const basket = prev.basket;
          const nearBasket =
            Math.abs(updatedBall.x - (basket.x + basket.width / 2)) <
              basket.width && Math.abs(updatedBall.y - basket.y) < 30;
  ...
  const handleCanvasPress = (event: any) => {
    ...
    const constrainedX = Math.max(
      POINT_RADIUS,
      Math.min(drawAreaWidth - POINT_RADIUS, locationX)
    );
    const constrainedY = Math.max(
      POINT_RADIUS,
      Math.min(drawAreaHeight - POINT_RADIUS, locationY)
    );
  ...
  const findNearestPoint = (x: number, y: number): Point | null => {
    ...
    let minDistance = POINT_RADIUS * 3; // Use smaller threshold to prevent overlapping
  ...
  // Helper function for line drawing with larger touch area
  const findNearestPointForLineDrawing = (
    x: number,
    y: number
  ): Point | null => {
    let nearest: Point | null = null;
    let minDistance = TOUCH_RADIUS; // Larger area for easier line drawing
  ...
  const panResponder = PanResponder.create({
    onStartShouldSetPanResponder: (event) => {
      ...
      if (gameState.points.length >= 2) {
        const nearestPoint = findNearestPointForLineDrawing(
          locationX,
          locationY
        );
    ...
    onPanResponderRelease: (event) => {
      if (isDrawing && currentStart) {
        ...
        if (nearestPoint && nearestPoint.id !== currentStart.id) {
          const lineExists = gameState.lines.some(
            (line) =>
              (line.start.id === currentStart.id &&
                line.end.id === nearestPoint.id) ||
              (line.start.id === nearestPoint.id &&
                line.end.id === currentStart.id)
          );
          ...
          <View style={styles.tutorialContent}>
            <View style={styles.tutorialStep}>
              ...
              <Text style={styles.stepText}>
                <Text style={styles.boldText}>Tạo điểm:</Text> Chạm vào lưới để
                đặt các điểm
              </Text>
            </View>
            ...
            <View style={styles.tutorialStep}>
              ...
              <Text style={styles.stepText}>
                <Text style={styles.boldText}>Vẽ đường:</Text> Kéo từ điểm này
                đến điểm khác (cần ít nhất 2 điểm)
              </Text>
            </View>
            ...
            <View style={styles.tutorialStep}>
              ...
              <Text style={styles.stepText}>
                <Text style={styles.boldText}>Thả bóng:</Text> Nhấn &quot;Bắt
                đầu&quot; để thả bóng từ vị trí ngẫu nhiên
              </Text>
            </View>
            ...
            <View style={styles.tutorialStep}>
              ...
              <Text style={styles.stepText}>
                <Text style={styles.boldText}>Mục tiêu:</Text> Bóng phải chạm
                đường thẳng và rơi vào rổ!
              </Text>
            </View>
          </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!hasSeenGameTutorial && (
        <View style={styles.instructionContainer}>
          <Text style={styles.instructionText}>
            🏀 Tạo điểm và vẽ đường thẳng để dẫn bóng vào rổ! Bóng sẽ tự động
            rơi từ trên xuống.
          </Text>
          <Text style={styles.hintText}>
      ...
      <View style={styles.drawingArea}>
        ...
        >
          <View style={styles.canvasContent} {...panResponder.panHandlers}>
            <Svg
              width={drawAreaWidth}
            ...
            >
              ...
              {gameState.points.map((point: Point) => (
                <Circle
                  ...
                  r={POINT_RADIUS}
                  fill={
                    currentStart && currentStart.id === point.id
                      ? "#e74c3c"
                      : "#3498db"
                  }
            ...
            {/* Canvas hints - hide after user has progressed or started playing */}
