...
export default function FixedMode() {
  ...
  const titleText = useMemo(() => {
    if (!total) return "Cố định";
    return `Cố định (${engine.ui.question}/${total})`;
  }, [total, engine.ui.question]);

  const gameOverTitle =
    engine.ui.endReason === "done" ? "Hoàn thành!" : "Kết thúc!";
      ...
      {!total ? (
        ...
        <View style={{ flex: 1, justifyContent: "center", gap: 14 }}>
          ...
          <Text style={styles.tip}>
            (mix) sẽ tự tăng độ khó theo tiến trình câu hỏi.
          </Text>
        </View>
      )}
    </ScreenShell>
