...
export default function ChapterDetailScreen() {
  ...
  if (!currentChapter) {
    return (
      <SafeAreaView style={styles.container}>
        <Text style={styles.errorText}>Không tìm thấy nội dung cho Chapter {chapterId}</Text>
        <TouchableOpacity 
  ...
  const renderLesson = ({ item }: { item: any }) => (
    ...
    >
      ...
      <View style={[styles.statusBadge, { backgroundColor: item.completed ? '#4CAF50' : '#FF9800' }]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.chapterTitle}>Chapter {chapterId}: {currentChapter.title}</Text>
...
const styles = StyleSheet.create({
  ...
  lessonList: { flex: 1 },
  lessonCard: { backgroundColor: 'white', padding: 15, marginBottom: 10, borderRadius: 10, shadowOpacity: 0.1, shadowRadius: 5, elevation: 3 },
  lessonTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 5 },
  lessonDescription: { fontSize: 14, color: '#666', marginBottom: 10 },
  statusBadge: { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 15 },
  statusText: { color: 'white', fontSize: 12, fontWeight: 'bold' },
  errorText: { fontSize: 18, color: 'red', textAlign: 'center', marginTop: 50 },
  backButton: { backgroundColor: '#2196F3', padding: 15, margin: 20, borderRadius: 10 },
  backButtonText: { color: 'white', fontSize: 16, textAlign: 'center', fontWeight: 'bold' }
});